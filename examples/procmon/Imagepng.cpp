...
Imagepng::Imagepng(const std::string& filename, Type t, int width, int height) :
    filename_(filename),
    type_(t),
    width_(width),
    height_(height),
    ...
    color_(NULL)
{
    ...
    rgb_   = new unsigned char[width_*height_*3];
    rgba_  = new unsigned char[width_*height_*4];
    color_ = new Color[width_ * height_];
    memset(color_, 255, sizeof(Color) * width_ * height_);
}
...
void Imagepng::save()
{
    ...
    if (type_ == RGB) {
        p = rgb_;
        for (int i=0; i<width_*height_; i++,p+=3) {
            *(p+0) = color_[i].R;
            *(p+1) = color_[i].G;
            *(p+2) = color_[i].B;
        }
        svpng(fp_, width_, height_, rgb_, 0);
    } else {
        p = rgba_;
        for (int i=0; i<width_*height_; i++,p+=4) {
            *(p+0) = color_[i].R;
            *(p+1) = color_[i].G;
            *(p+2) = color_[i].B;
            *(p+3) = color_[i].A;
        }
        svpng(fp_, width_, height_, rgba_, 1);
    }
}